...
'''
import subprocess
from urllib.parse import quote ##將網頁的中文字encode
import os
...
std_output2 = subprocess.run(cmd)
if std_output2.returncode==0:
...
for index,q in enumerate(list_words):
    ...
    std_output2 = subprocess.run(cmd2)

    if std_output2.returncode==0:
        print(f'{index}_atempo.mp3下載成功')
    else:
        print(f'{index}_atempo.mp3下載失敗')
